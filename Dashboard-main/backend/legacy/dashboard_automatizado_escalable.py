...
def delete_estudio(estudio_id):
    ...
    c = conn.cursor()
    try:
        c.execute("DELETE FROM estudios WHERE id = ?", (int(estudio_id),))
        c.execute("DELETE FROM cuestionarios WHERE estudio_id = ?", (int(estudio_id),))
        conn.commit()
        return True
    except Exception:
        return False
...
def calcular_nps_y_avance(df_respuestas):
    ...
    df_respuestas = pd.merge(df_respuestas, df_nps[['nps_score', 'nps_categoria']], on='nps_score', how='left', suffixes=('', '_new'))
    df_respuestas['nps_categoria'] = df_respuestas['nps_categoria_new'].fillna('No Aplica')
    df_respuestas = df_respuestas.drop(columns=['nps_categoria_new'])
...
def set_theme():
    ...
    LIGHT_BLUE_BACKGROUND_START = "#E0F2F7" 
    ...
    LIGHT_ELEMENT_BACKGROUND = "#FFFFFF" 
...
def home_page():
    ...
    if df_estudios_db.empty:
        ...
        # --- ESTRUCTURA DE TABLA SIMULADA CON st.columns ---
        with st.container():
            ...
            for index, row in df_display.iterrows():
                # Usamos un container con border para simular una fila tipo "tarjeta"
                with st.container():
                    st.markdown("<div style='border-bottom: 1px solid #303030; padding: 5px 0;'>", unsafe_allow_html=True)
                    col_id, col_titulo, col_fecha, col_estado, col_monitor, col_acciones = st.columns([0.4, 2.5, 1.2, 1.2, 1.2, 0.5])
                    estudio_id = row['ID']
                    with col_id:
                    ...
                    with col_acciones:
                        if st.button("🗑️", key=f"delete_{estudio_id}", help=f"Eliminar estudio ID: {estudio_id}"):
                            if delete_estudio(estudio_id):
                                st.toast(f"🗑️ Estudio ID {estudio_id} eliminado correctamente.")
                                st.cache_data.clear()
                                st.rerun()
                            else:
                                st.error(f"No se pudo eliminar el estudio ID {estudio_id}.")
        ...
        # Filtrar solo IDs y Títulos para la selección
        ...
        # Lógica de ruteo para el estudio seleccionado
        if selected_display:
...
def crear_estudio_page():
    ...
    with st.form("nuevo_estudio_form"):
        ...
        tecnica = st.selectbox("Selecciona la Técnica", ['CATI (Call Center)', 'CAPI (Link Web)', 'IVR (Contestada Automática)', 'PAPI (Presencial)'], key='f_tecnica')
...
def analizar_resultados_page():
    """Página para el análisis de resultados (NPS, tabulaciones, test de hipótesis)."""
    ...
    if df_respuestas is None:
        st.warning("No hay datos de respuesta cargados en la sesión para este estudio.")
        uploaded_file = st.file_uploader("Carga el archivo CSV de resultados del estudio para el análisis.", type=['csv'], key='analisis_upload')
        if uploaded_file:
            try:
    ...
    with tab1:
        ...
        st.markdown("---")
        st.subheader("Tabulación por Pregunta (Simple)")
        
        # Filtrar preguntas para tabulación simple (excluir NPS, ID y Abiertas)
        preguntas_tabulables = [q for q in preguntas if q['tipo'] not in ['NPS', 'ID', 'Abierta (Texto)']]
        opciones_q = {q['texto']: q['columna'] for q in preguntas_tabulables}
        ...
        if col_csv in df_respuestas.columns:
            ...
            # 1. Calcular frecuencias (Convertir a string para conteo consistente)
    ...
    with tab2:
        st.subheader("Tabulaciones Cruzadas (Crosstabs)")
        st.write("Selecciona una pregunta por fila y otra por columna para ver la distribución cruzada.")
        
        # Columnas válidas: categóricas con más de 1 valor y menos de 50
        columnas_validas_q = [q for q in preguntas if q['tipo'] in ['Single (Opción Única)', 'Demográfico', 'NPS']]
        columnas_validas = [q['columna'] for q in columnas_validas_q if df_respuestas[q['columna']].nunique() > 1 and df_respuestas[q['columna']].nunique() <= 50]
        
        if len(columnas_validas) < 2:
            st.warning("Necesitas al menos dos preguntas categóricas con baja varianza para hacer un Crosstab.")
            return

        col_fila, col_col = st.columns(2)
        with col_fila:
            # Intenta usar 'genero' como valor por defecto si existe
            default_fila = columnas_validas.index('genero') if 'genero' in columnas_validas else 0
            var_fila = st.selectbox("Variable de Fila", columnas_validas, index=default_fila)
        
        with col_col:
            # Intenta usar 'nps_categoria' o la siguiente
            default_columna = columnas_validas.index('nps_categoria') if 'nps_categoria' in columnas_validas else (default_fila + 1) % len(columnas_validas)
            var_columna = st.selectbox("Variable de Columna", columnas_validas, index=default_columna)
        
        if var_fila and var_columna:
            crosstab = pd.crosstab(df_respuestas[var_fila], df_respuestas[var_columna], margins=True, normalize=False)
            st.markdown("#### Frecuencias Absolutas (N)")
            st.dataframe(crosstab.astype(int), use_container_width=True)
            
            st.markdown("#### Porcentajes por Fila")
            # Normalizar por fila (índice 0) y multiplicar por 100
            crosstab_pct_fila = pd.crosstab(df_respuestas[var_fila], df_respuestas[var_columna], margins=True, normalize='index') * 100
            st.dataframe(crosstab_pct_fila.round(1), use_container_width=True)
            
            st.markdown("#### Porcentajes por Columna")
            # Normalizar por columna (índice 1) y multiplicar por 100
            crosstab_pct_columna = pd.crosstab(df_respuestas[var_fila], df_respuestas[var_columna], margins=True, normalize='columns') * 100
            st.dataframe(crosstab_pct_columna.round(1), use_container_width=True)

            # Gráfico de barras apiladas (100% por fila)
            df_plot = crosstab_pct_fila.reset_index().melt(id_vars=var_fila, var_name=var_columna, value_name='Porcentaje')
            fig_cross = px.bar(
                df_plot,
                x=var_fila,
                y='Porcentaje',
                color=var_columna,
                title=f'Distribución de {var_columna} por {var_fila} (Porcentaje por Fila)',
                height=500
            )
    ...
    with tab3:
        ...
        preguntas_num = [q for q in preguntas if q['tipo'] in ['NPS', 'Calculada/Auxiliar'] or 'score' in q['columna']]
        preguntas_cat = [q for q in preguntas if q['tipo'] in ['Single (Opción Única)', 'Demográfico'] and df_respuestas[q['columna']].nunique() == 2]

        opciones_num = {q['texto']: q['columna'] for q in preguntas_num}
        opciones_cat = {q['texto']: q['columna'] for q in preguntas_cat}
    ...
    with tab4:
        ...
        st.write("Genera un archivo CSV de la base de datos cargada. Idealmente, esto incluiría un diccionario de variables (pendiente de implementación en este prototipo).")
        
        if df_respuestas is not None and not df_respuestas.empty:
            csv_file = convert_df_to_csv(df_respuestas)
            st.download_button(
                label="Descargar Base de Datos CSV",
                data=csv_file,
                file_name=f'base_datos_estudio_{estudio_id}_{datetime.now().strftime("%Y%m%d")}.csv',
                mime='text/csv',
                type="primary"
            )
        else:
            st.info("No hay datos cargados en la sesión para descargar.")
...
def codificacion_abiertas_page():
    ...
    preguntas = get_questions_by_study(estudio_id)
    opciones_abiertas = {q['texto']: q['columna'] for q in preguntas if q['tipo'] == 'Abierta (Texto)'}
            ...
            if submit_button:
                if id_externo and nombre:
                    if save_encuestado(id_externo, nombre, comuna, edad, genero):
                        st.success(f"✅ Encuestado '{nombre}' (ID: {id_externo}) registrado con éxito.")
                        st.rerun()
                    else:
