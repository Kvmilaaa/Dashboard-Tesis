...
if 'username' not in st.session_state:
    ...
    if 'rol' not in st.session_state:
     ...
     rol_actual = st.session_state.get('rol') or 'Cliente'
...
def init_db():
    ...
    # Migración para bases de datos antiguas que aún no tengan la columna rol
    try:
        c.execute(
            "ALTER TABLE users ADD COLUMN rol TEXT NOT NULL DEFAULT 'Cliente'"
        )
    ...
    # Migración para bases antiguas que aún no tengan link_cuestionario
    try:
        c.execute(
            "ALTER TABLE estudios ADD COLUMN link_cuestionario TEXT"
        )
    except sqlite3.OperationalError:
        pass
...
def create_user(username, password, nombre, correo, rol="Cliente"):
    ...
    try:
        c.execute(
            """
            INSERT INTO users
            (username, password_hash, nombre_completo, correo, rol)
            VALUES (?, ?, ?, ?, ?)
            """,
            (
                username,
...
def get_all_users():
    ...
    c.execute("""
        SELECT username, nombre_completo, correo, rol
        FROM users
        ORDER BY username
    """)
...
def update_user_role(username, nuevo_rol):
    if nuevo_rol not in ["Cliente", "Analista", "Administrador"]:
        return False
...
def verify_user(username, password):
    ...
    c.execute(
        "SELECT password_hash, rol FROM users WHERE username = ?",
        (username,)
    )
...
def delete_estudio(estudio_id):
    ...
    try:
        # Primero elimina las preguntas asociadas al estudio
        c.execute("DELETE FROM cuestionarios WHERE estudio_id = ?", (estudio_id,))

        # Luego elimina el estudio
        c.execute("DELETE FROM estudios WHERE id = ?", (estudio_id,))

        conn.commit()

        # Limpia la caché para actualizar la lista de estudios
        st.cache_data.clear()
...
def calcular_nps_y_avance(df_respuestas):
    ...
    # Añadir la categoría NPS calculada al DataFrame original
    df_respuestas['nps_categoria'] = 'No Aplica'

    df_respuestas.loc[df_nps.index, 'nps_categoria'] = df_nps['nps_categoria'] 
...
def reset_password_page():
    """Permite solicitar de forma segura la recuperación de contraseña."""
    
    st.title("🔐 Recuperación de Contraseña")

    st.info(
        "Por razones de seguridad, la contraseña no puede restablecerse "
        "directamente desde esta pantalla."
    )

    st.write(
        "Ingresa tu nombre de usuario y el correo registrado en tu cuenta. "
        "Si los datos coinciden, se enviará una solicitud al Administrador."
    )

    with st.form("password_reset_request_form"):
        username_reset = st.text_input("Nombre de Usuario")
        correo_reset = st.text_input("Correo registrado")

        enviar_solicitud = st.form_submit_button(
            "📨 Enviar solicitud de recuperación"
        )

        if enviar_solicitud:
            username_reset = username_reset.strip()
            correo_reset = correo_reset.strip()

            if not username_reset or not correo_reset:
                ...
                # Verificar que usuario y correo correspondan a la misma cuenta
                c.execute(
                    """
                    SELECT username
                    FROM users
                ...
                if usuario:
                    ...
                    c.execute(
                        ...
                        WHERE username = ? AND estado = 'Pendiente'
                        """,
                        (username_reset,)
                    )

                    solicitud_existente = c.fetchone()

                    if solicitud_existente:
                        st.warning(
                            "⚠️ Ya existe una solicitud de recuperación "
                            "pendiente para esta cuenta."
                        )
                        ...
                        st.success(
                            ...
                            "Un Administrador deberá verificarla y "
                            "asignar una contraseña temporal."
                        )

                else:
                    st.error(
                        "❌ El usuario y el correo ingresados no coinciden "
...
def gestion_usuarios_page():
    ...
    # Seguridad: solo Administrador puede entrar
    ...
    st.write("Desde aquí puedes asignar roles a los usuarios registrados.")

    for username, nombre, correo, rol in usuarios:
        st.markdown("---")
        ...
        with col3:
            st.write(f"**Correo:** {correo or '-'}")

        with col4:
            roles = ["Cliente", "Analista", "Administrador"]

            nuevo_rol = st.selectbox(
                "Rol",
                roles,
                index=roles.index(rol) if rol in roles else 0,
                key=f"rol_{username}"
            )

            if nuevo_rol != rol:
                if st.button("Guardar", key=f"guardar_{username}"):
                    if update_user_role(username, nuevo_rol):
                        st.success(f"Rol de {username} actualizado correctamente.")
                        st.rerun()
                    else:
    ...
    # --------------------------------------------------
    st.markdown("---")
    st.subheader("🔐 Solicitudes de recuperación de contraseña")
    ...
    c.execute(
        """
        SELECT id, username, correo, fecha_solicitud, estado
        FROM password_reset_requests
    ...
    if not solicitudes:
        st.info("No hay solicitudes de recuperación pendientes.")
    else:
        for solicitud_id, username_sol, correo_sol, fecha_sol, estado_sol in solicitudes:
            st.markdown("---")

            st.write(f"**Usuario:** {username_sol}")
            st.write(f"**Correo registrado:** {correo_sol}")
            st.write(f"**Fecha de solicitud:** {fecha_sol}")
            st.write(f"**Estado:** {estado_sol}")

            nueva_password_temp = st.text_input(
                "Contraseña temporal",
                type="password",
                key=f"temp_pass_{solicitud_id}"
            )

            confirmar_password_temp = st.text_input(
                "Confirmar contraseña temporal",
                type="password",
                key=f"temp_pass_confirm_{solicitud_id}"
            )

            if st.button(
                "✅ Restablecer contraseña",
                key=f"reset_pass_{solicitud_id}"
            ):
                if not nueva_password_temp or not confirmar_password_temp:
                    st.error("Debes completar ambos campos de contraseña.")

                elif nueva_password_temp != confirmar_password_temp:
                    st.error("Las contraseñas no coinciden.")

                else:
                    hashed_password = bcrypt.hashpw(
                        nueva_password_temp.encode("utf-8"),
                        bcrypt.gensalt()
                    ).decode("utf-8")
                    ...
                    st.success(
                        f"Contraseña temporal asignada correctamente a {username_sol}."
                    )
...
def home_page():
    ...
    if rol_actual in ['Analista', 'Administrador']:
    ...
    if df_estudios_db.empty:
        ...
        with col_acciones:
         if rol_actual == 'Administrador':
          if st.button("🗑", key=f"del_{estudio_id}"):
...
def ver_detalle_page(estudio_id):
    ...
    with col2:
        ...
        # Analista y Administrador pueden cambiar el estado
        if rol_actual in ['Analista', 'Administrador']:
            state_options = [
                'Creación de Estudio',
                'Campo Activo',
        ...
        # Cliente solo puede ver el estado
        else:
            st.write("**Estado actual:**")
            st.info(estudio_details['estado'])
...
def mostrar_botones_estudio_activo():
    ...
    if rol_actual == 'Cliente':
        ...
        with col1:
            if st.button("← Volver a Inicio", key="nav_home_sup"):
                st.session_state['current_view'] = 'home'
                st.rerun()

        with col2:
            if st.button("👁️ Ver Detalles", key="nav_detalles_sup"):
                st.session_state['current_view'] = 'ver_detalle'
                st.rerun()
...
def analizar_resultados_page():
   ...
   # Obtener el ID del estudio actual para mantener los datos separados por estudio
    ...
    col_nps, col_pro, col_det, col_pas = st.columns(4)
    ...
    with col_pro:
        st.metric(
            ...
            value=f"{percent_promotores:.1f}%"
        )

    with col_det:
        st.metric(
            label="% Detractores (0-6)",
            value=f"{percent_detractores:.1f}%"
        )

    with col_pas:
        pasivos = 100 - percent_promotores - percent_detractores
        st.metric(
            label="% Pasivos (7-8)",
            value=f"{pasivos:.1f}%"
        )
    ...
    if total_validas > 0:
        ...
        # Asegurar que todas las categorías estén presentes para el gráfico
        categorias_nps = ['Promotor', 'Pasivo', 'Detractor', 'No Aplica']

        for cat in categorias_nps:
            if cat not in df_nps_counts['Categoría'].values:
                df_nps_counts.loc[len(df_nps_counts)] = {
                    'Categoría': cat,
                    'N': 0
                }
   ...
   # Filtrar preguntas para tabulación simple (excluir NPS, ID y Abiertas)
    ...
    with tab3:
        ...
        preguntas_num = [
            ...
            if q.get('tipo', q.get('tipo_pregunta', '')) in ['NPS', 'Calculada/Auxiliar']
            or 'score' in q.get('columna', q.get('columna_csv', ''))
        ]

        preguntas_cat = [
            q for q in preguntas
            if q.get('tipo', q.get('tipo_pregunta', '')) in ['Single (Opción Única)', 'Demográfico']
        ]

        opciones_num = {
            q['pregunta_texto']: q['columna_csv']
            for q in preguntas_num
        }

        opciones_cat = {
            q['pregunta_texto']: q['columna_csv']
            for q in preguntas_cat
        }
        ...
        with col_num_sel:
            col_num_text = st.selectbox(
                "Variable Numérica (Dependiente)",
                list(opciones_num.keys())
            )
            col_num = opciones_num[col_num_text]

        with col_cat_sel:
            col_cat_text = st.selectbox(
                "Variable Categórica (Independiente - Binaria)",
            ...
            col_cat = opciones_cat[col_cat_text]
               ...
               # 2. Ejecutar T-Test
        try:
            categorias = df_respuestas[col_cat].dropna().unique().tolist()

            if len(categorias) < 2:
                st.error(
                    f"La variable categórica '{col_cat_text}' debe contener al menos dos grupos para realizar el T-Test."
                )
                return

            # Si existen más de dos categorías, permitir seleccionar qué dos grupos comparar
            if len(categorias) > 2:
                st.info(
                    f"La variable '{col_cat_text}' contiene {len(categorias)} categorías. "
                    "Selecciona los dos grupos que deseas comparar."
                )
                ...
                with col_g2:
                    opciones_grupo2 = [g for g in categorias if g != grupo1]
                    grupo2 = st.selectbox(
                        "Grupo 2",
                        opciones_grupo2,
                        key="ttest_grupo2"
            ...
            # Limpiar datos y eliminar valores faltantes
            df_test = df_respuestas[[col_num, col_cat]].copy()
            df_test[col_num] = pd.to_numeric(df_test[col_num], errors="coerce")
            df_test = df_test.dropna(subset=[col_num, col_cat])

            # Seleccionar únicamente los dos grupos elegidos
            ...
            if len(data_g1) < 2 or len(data_g2) < 2:
                st.warning(
                    "No hay suficientes datos válidos en al menos uno de los grupos "
                    "para realizar la prueba t de Welch."
                )
                return
            ...
            # Cálculo de grados de libertad de Welch-Satterthwaite
            n1 = len(data_g1)
            n2 = len(data_g2)

            var1 = data_g1.var(ddof=1)
            var2 = data_g2.var(ddof=1)
        ...
        st.markdown("---")
        st.subheader("Resultados de la prueba t de Welch")
        ...
        st.markdown(
            ...
            **H0:** La media de **{col_num_text}** es igual entre **{grupo1}** y **{grupo2}**.

            **H1:** La media de **{col_num_text}** es diferente entre **{grupo1}** y **{grupo2}**.
            """
        )

        col_t, col_p, col_gl = st.columns(3)

        with col_t:
            st.metric("Estadístico t", f"{t_stat:.3f}")

        with col_p:
            st.metric("Valor p", f"{p_value:.4f}")

        with col_gl:
            st.metric("Grados de libertad", f"{grados_libertad:.2f}")
        ...
        st.write(f"**Nivel de significancia:** α = {alpha}")

        if p_value < alpha:
            st.success(
                f"Se rechaza H0 (p = {p_value:.4f} < {alpha}). "
                f"Existe evidencia estadística de una diferencia entre las medias "
                f"de {grupo1} y {grupo2} para la variable '{col_num_text}'."
            )
        else:
            st.info(
                f"No se rechaza H0 (p = {p_value:.4f} ≥ {alpha}). "
                f"No existe evidencia estadística suficiente para afirmar que las medias "
                f"de {grupo1} y {grupo2} sean diferentes para la variable '{col_num_text}'."
            )
    ...
    if tab4 is not None:
     with tab4:
        ...
        # Solo Analista y Administrador pueden descargar la base completa
        if rol_actual in ['Analista', 'Administrador']:
            ...
            if df_download is not None and not df_download.empty:
                ...
                st.info(
                    "ℹ️ No hay datos cargados en la sesión para este estudio. "
                    "Carga un estudio en el Home para habilitar la descarga."
                )

        else:
            st.info(
                "🔒 La descarga de la base completa está disponible solo para "
                "usuarios con rol Analista o Administrador."
            )
...
def codificacion_abiertas_page():
   ...
   if any(term in str(q.get('tipo', q.get('tipo_pregunta', ''))).lower() for term in ['abierta', 'texto', 'open'])
    ...
    if not st.session_state['coding_codes']:
        ...
        # Crear una muestra fija para evitar que cambie en cada interacción
        if 'muestra_codificacion' not in st.session_state:
            st.session_state['muestra_codificacion'] = df_respuestas.sample(
                min(10, len(df_respuestas)),
                random_state=42
            )

        muestra_codificar = st.session_state['muestra_codificacion']

        st.info(
            f"Muestra de {len(muestra_codificar)} respuestas para codificar:"
        )

        # Diccionario para conservar las codificaciones realizadas
        if 'codificaciones_manual' not in st.session_state:
            st.session_state['codificaciones_manual'] = {}
        ...
        for index, row in muestra_codificar.iterrows():
            ...
            seleccion = st.multiselect(
                "Asignar Códigos",
                options=st.session_state['coding_codes'],
                default=st.session_state['codificaciones_manual'].get(
                    str(index), []
                ),
                key=f"code_select_{index}"
            )

            # Guardar la codificación de esta respuesta
            st.session_state['codificaciones_manual'][str(index)] = seleccion

            if seleccion:
                st.success(
                    "Código(s) asignado(s): " + ", ".join(seleccion)
                )
       ...
       # 4. Resultados de la Codificación
    st.subheader("4. Resultados de la Codificación")

    # Reunir todos los códigos asignados
    codigos_asignados = []

    for codigos in st.session_state.get('codificaciones_manual', {}).values():
        codigos_asignados.extend(codigos)
    ...
    if codigos_asignados:
        ...
        conteo = Counter(codigos_asignados)
        total_codificaciones = sum(conteo.values())

        # Crear tabla de resultados
        resultados_codificacion = pd.DataFrame({
            "Código": list(conteo.keys()),
            "Frecuencia": list(conteo.values())
        })

        # Calcular porcentaje
        resultados_codificacion["Porcentaje"] = (
            resultados_codificacion["Frecuencia"]
            / total_codificaciones * 100
        ).round(1)

        resultados_codificacion["Porcentaje"] = (
            resultados_codificacion["Porcentaje"].astype(str) + " %"
        )

        # Mostrar tabla
        st.markdown("#### Resumen de códigos asignados")
        ...
        st.markdown("#### Distribución de códigos")

        datos_grafico = resultados_codificacion.copy()
        ...
        barras = ax.barh(
            datos_grafico["Código"],
            datos_grafico["Frecuencia"]
        )
        ...
        ax.set_title("Distribución de códigos asignados")
        ...
        # Mensaje final
        st.success(
            f"Se han realizado {total_codificaciones} "
            "asignaciones de códigos en la muestra analizada."
        )

    else:
        st.info(
            "Asigna códigos a las respuestas para generar "
...
if not st.session_state['logged_in']:
    ...
    with st.sidebar:
        ...
        # Solo Administrador
        if rol_actual == 'Administrador':
            if st.button("👥 Gestión de Usuarios", key="nav_usuarios"):
                st.session_state['current_view'] = 'gestion_usuarios'
                st.rerun()
    ...
    if current_view == 'home':
        ...
        if rol_actual in ['Analista', 'Administrador']:
            ...
            st.error("⛔ No tienes permisos para crear estudios.")
            st.session_state['current_view'] = 'home'
        ...
        if rol_actual not in ['Analista', 'Administrador']:
            st.error("⛔ No tienes permisos para crear o modificar cuestionarios.")
            st.session_state['current_view'] = 'home'
        elif selected_id:
        ...
        if rol_actual in ['Analista', 'Administrador']:
            ...
            st.error("⛔ No tienes permisos para acceder a la codificación de respuestas.")
            st.session_state['current_view'] = 'home'
        ...
        if rol_actual in ['Analista', 'Administrador']:
            ...
            st.error("⛔ No tienes permisos para gestionar paneles.")
            st.session_state['current_view'] = 'home'

    elif current_view == 'gestion_usuarios':
        if rol_actual == 'Administrador':
